...
func FindMatchmakeSessionBySearchCriteria(manager *common_globals.MatchmakingManager, connection *nex.PRUDPConnection, searchCriterias []match_making_types.MatchmakeSessionSearchCriteria, resultRange types.ResultRange, sourceMatchmakeSession *match_making_types.MatchmakeSession) ([]match_making_types.MatchmakeSession, *nex.Error) {
	...
	if nexErr != nil {
		common_globals.Logger.Errorf("Failed to get own blocklist for PID: %d, Error: %s", connection.PID(), nexErr.Error())
		return nil, nexErr
	}
	blockedByList, nexErr := GetBlockedByList(manager, connection.PID())
	if nexErr != nil {
		common_globals.Logger.Errorf("Failed to get list of users blocking PID: %d, Error: %s", connection.PID(), nexErr.Error())
		return nil, nexErr
	}
